 ...
 *
 */
const rp = (opts, retryCounter, retryNum, retryWait) => {
  return needle(opts.method.toLowerCase(), // needle takes e.g. 'put' not 'PUT'
    opts.url,
    opts.body || opts.params,
    opts)
    .then(resp => {
      if ((resp.statusCode === 429) && (retryCounter < retryNum)) {
        // retry after waiting
        return sleep(retryWait).then(() =>
          rp(opts, retryCounter + 1, retryNum, retryWait * 2)
        )
...
class Client {
   ...
   * @param {number} [options.retry_num]
   * @param {number} [options.retry_wait]
   */
  constructor (options) {
  ...
  parseOptions (options) {
    ...
    const retryNum = options.retry_num || 1
    const retryWait = options.retry_wait || 100

    return { apiKey: apiKey, api, apiVersion: apiversion, ignoreCerts: ignoreCerts, namespace: options.namespace, apigwToken: apigwToken, apigwSpaceGuid: apigwSpaceGuid, authHandler: options.auth_handler, noUserAgent: options.noUserAgent, cert: options.cert, key: options.key, proxy, agent, retryNum, retryWait }
  }
  ...
  request (method, path, options) {
    const retryNum = options.retry_num || this.options.retryNum
    const retryWait = options.retry_wait || this.options.retryWait
    const params = this.params(method, path, options)
    return params.then(req => {
      if (req.qs) {
        // we turn the qs struct into a query string over the url
        let first = true
        for (let key in req.qs) {
          const str = `${encodeURIComponent(key)}=${encodeURIComponent(req.qs[key])}`
          if (first) {
            req.url += `?${str}`
            first = false
          } else {
      ...
      // req.json field to true; rp is apparently more resilient to
      // this situation than needle
      req.json = true
      return rp(req, 1, retryNum, retryWait)
